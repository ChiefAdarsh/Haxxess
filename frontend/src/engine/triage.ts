...
export function triageSymptoms(symptoms: SymptomEntry[]): TriageResult {
  ...
  if (recent.length === 0)
    return { level: "self_care", reasons: ["no recent symptoms"] };
  ...
  const hasQuality = (q: string) =>
    recent.some((s) => s.qualities.includes(q as any));
  ...
  if (
    has("bleeding") &&
    recent.some((s) => s.type === "bleeding" && s.severity >= 8)
  ) {
    level = "emergency";
  ...
  if (
    has("bleeding") &&
    (hasNoteMatch("dizzy") || hasNoteMatch("faint") || hasNoteMatch("syncope"))
  ) {
    level = "emergency";
  ...
  if (
    hasNoteMatch("fever") &&
    (hasRegion("pelvic_midline") || has("discharge"))
  ) {
...
export const triageLevelConfig: Record<
  TriageLevel,
