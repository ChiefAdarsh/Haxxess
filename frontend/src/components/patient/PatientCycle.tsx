...
export default function PatientCycle() {
      ...
      try {
        ...
        if (!cancelled && data?.status === "success") {
          ...
          localStorage.setItem(
            LS_KEY,
            JSON.stringify({ periods: data.periods || [] }),
          );
          setLoading(false);
          return;
        }
  ...
  const computed = useMemo(() => {
    ...
    const periodsSorted = [...periods].sort((a, b) =>
      a.startDate.localeCompare(b.startDate),
    );
    const last = periodsSorted[periodsSorted.length - 1];
    if (!last?.startDate) return null;

    const cycleLength = stats.median_cycle_length
      ? Math.round(stats.median_cycle_length)
      : 28;
    ...
    const periodLength = stats.median_period_length
      ? Math.round(stats.median_period_length)
      : 5;
    ...
    else if (day >= ovulationDayApprox - 1 && day <= ovulationDayApprox + 1)
      phase = "Ovulation";
    else if (day > ovulationDayApprox + 1) phase = "Luteal";

    const phaseInfo: Record<
      typeof phase,
      { subtitle: string; symptoms: string[] }
    > = {
      Menstrual: {
        subtitle:
          "Shedding the uterine lining. Energy may dip; cramps can be more common.",
        symptoms: [
          "cramping",
          "fatigue",
          "bloating",
      ...
      Ovulation: {
        subtitle:
          "Ovulation window. Some people notice one-sided pelvic pain or increased discharge.",
        symptoms: [
      ...
      Luteal: {
        subtitle:
          "Progesterone rises. PMS-type symptoms may increase for some.",
        symptoms: [
          "breast tenderness",
          "mood changes",
          "bloating",
  ...
  const addPeriod = async () => {
    ...
    const optimistic = [...periods, { ...draft }].sort((a, b) =>
      a.startDate.localeCompare(b.startDate),
    );
    ...
    try {
      ...
      if (data?.status === "success") {
        ...
        localStorage.setItem(
          LS_KEY,
          JSON.stringify({ periods: data.periods || optimistic }),
        );
      }
    } catch {
  ...
  return (
    <div>
      ...
      >
        Log periods over time — Vitality learns your typical cycle length and
        variability, then forecasts your next cycle phase.
      </p>
      ...
      >
          ...
          <label>
            ...
            <select
              ...
              onChange={(e) =>
                setDraft((d) => ({
                  ...d,
                  flow: (e.target.value || undefined) as any,
                }))
              }
        ...
        >
          ...
          <span style={{ fontSize: 12, color: "#9ca3af" }}>
            Add at least 2 periods to personalize predictions.
          </span>
        </div>
      </div>
      ...
      >
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 6 }}>
                {computed
                  ? `day of ${computed.cycleLength}`
                  : "add a period to start"}
              </div>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <p style={{ margin: 0, color: "#6b7280", fontSize: 13 }}>
            {computed
              ? computed.phaseSubtitle
              : "Vitality will estimate your phase and learn patterns as you log more cycles."}
          </p>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {fmtDate(preds.next_period_start)}
            </p>
          </div>
          ...
          >
            ...
            <p style={{ margin: "6px 0 0", fontSize: 12, color: "#9ca3af" }}>
              {stats.variability_days !== null
                ? `variability ±${stats.variability_days}d`
                : "add 2+ cycles"}
            </p>
          </div>
        </div>
