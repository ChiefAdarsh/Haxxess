...
export default function VitalsView() {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      const sys = data.apple_watch?.blood_pressure?.systolic_mmhg ?? liveData.sys
      const dia = data.apple_watch?.blood_pressure?.diastolic_mmhg ?? liveData.dia
      ...
      setLiveData({
        hr, sys, dia, glucose, temp: currentTemp, glucoseTrend,
        status: 'connected', lastSynced: new Date().toLocaleTimeString()
      })
    ...
    ws.onclose = () => setLiveData(prev => ({ ...prev, status: 'disconnected' }))
  ...
  const vitals = [
    ...
    },
    {
      ...
      unit: 'mg/dL',
      trend: liveData.glucoseTrend.includes('rising') ? 'up' : liveData.glucoseTrend.includes('falling') ? 'down' : 'stable',
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, color: '#1f2937', margin: 0 }}>My Live Vitals</h2>
        <span style={{
          fontSize: 12, fontWeight: 600, padding: '4px 10px', borderRadius: 12,
          backgroundColor: liveData.status === 'connected' ? '#d1fae5' : '#fee2e2',
          color: liveData.status === 'connected' ? '#065f46' : '#991b1b'
        }}>
          {liveData.status === 'connected' ? `🟢 Live — Last synced: ${liveData.lastSynced}` : '🔴 Disconnected'}
        </span>
      </div>
        ...
        {vitals.map((v) => {
          ...
          return (
            <div key={v.label} style={{
              ...
              boxShadow: '0 1px 3px rgba(0,0,0,0.05)', transition: 'transform 0.2s ease', cursor: 'default'
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{
                    width: 40, height: 40, borderRadius: 10, backgroundColor: `${v.color}15`,
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                  <TIcon size={14} color={trendColor[v.trend as keyof typeof trendColor]} />
                  <span style={{ fontSize: 11, color: trendColor[v.trend as keyof typeof trendColor], fontWeight: 500 }}>{v.trendText}</span>
                </div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {days.map((d, idx) => (
                    <span key={idx} style={{ fontSize: 10, color: '#d1d5db' }}>{d}</span>
                  ))}
                </div>
              </div>
            </div>
