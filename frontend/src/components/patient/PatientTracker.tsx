...
export default function PatientTracker() {
  ...
  const summary = useMemo(() => {
    ...
    const maxSeverity = count
      ? Math.max(...recent7.map((s: any) => s.severity ?? 0))
      : 0;
    ...
    <div>
      ...
      >
        ...
        <Card
          ...
          sub={
            summary.topRegion
              ? `${summary.topRegionCount} entries`
      ...
      >
        ...
        >
          7-day severity trend (daily max)
        </h3>
        <div
          style={{
            display: "flex",
      ...
      >
        ...
        {recent7.length === 0 ? (
          <p style={{ fontSize: 13, color: "#9ca3af", margin: 0 }}>
            No reports logged yet — use the Body Map to create your first
            report.
          </p>
        ) : (
            ...
            {recent7.slice(0, 10).map((s: any, idx: number) => (
              ...
              >
                <div style={{ fontWeight: 600, color: "#1f2937" }}>
                  {regionLabels[s.region as BodyRegion] ?? s.region} •{" "}
                  {s.severity}/10
                </div>
