...
export default function SymptomHistory() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 600, color: '#1f2937', margin: '0 0 20px' }}>Pelvic Symptom History</h2>
      ...
      <div style={{
        ...
        {sortedRegions.length === 0 ? (
          <p style={{ fontSize: 13, color: '#6b7280', fontStyle: 'italic', margin: 0 }}>No symptoms logged in the last 30 days.</p>
        ) : (
              ...
              <div key={region} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1, height: 20, backgroundColor: '#f3f4f6', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{
                    ...
                    backgroundColor: '#ec4899', opacity: 0.8, // Pinkish theme
                  }} />
                </div>
      ...
      <div style={{
        ...
        {sortedSymptoms.length === 0 ? (
          <p style={{ fontSize: 13, color: '#6b7280', fontStyle: 'italic', margin: 0 }}>Your timeline is empty.</p>
        ) : (
              ...
              return (
                <div key={s.id} style={{ display: 'flex', gap: 14 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 20 }}>
                    ...
                    {!isLast && (
                      <div style={{ width: 2, flex: 1, minHeight: 40, backgroundColor: '#e5e7eb', margin: '4px 0' }} />
                    )}
                  </div>
                  ...
                  <div style={{ paddingBottom: isLast ? 0 : 20, flex: 1 }}>
                    ...
                    <p style={{ fontSize: 13, fontWeight: 500, color: '#1f2937', margin: '0 0 2px', textTransform: 'capitalize' }}>
                      {s.type} — {regionLabels[s.region] || s.region}
                    </p>
                    {s.qualities.length > 0 && (
                      <p style={{ fontSize: 12, color: '#6b7280', margin: '0 0 4px', textTransform: 'lowercase' }}>
                        {s.qualities.join(', ')}
                      </p>
                    )}
                    {s.notes && (
                      <p style={{
                        fontSize: 12, color: '#6b7280', margin: '4px 0 0', fontStyle: 'italic',
                        backgroundColor: '#f9fafb', padding: '6px 10px', borderRadius: 6, border: '1px solid #f3f4f6'
                      }}>
                        "{s.notes}"
                      </p>
                    )}
